...
class Key_keeper(object):
    """Key keeper"""
    _locksmith = None
    
    
    def __init__(self, locksmith: object=None):
        self._locksmith = locksmith
    
    
    def symmetrical_encryption(self, key: str, data: bytes):
        key = SHA256.new(key.encode())
    ...
    def symmetrical_decryption(self, key, cipher_pack):
        key = SHA256.new(key.encode())
        
        nonce = cipher_pack[:16]
        tag = cipher_pack[16:32]
        cipher_text = cipher_pack[32:]
        
        cipher = AES.new(key.digest(), AES.MODE_EAX, nonce)
        return cipher.decrypt_and_verify(cipher_text, tag)
    
    
    def asymmetrical_encryption(self, data: bytes):
        recipient_key = RSA.import_key(self._locksmith.get_public_key)
        session_key = get_random_bytes(16)
        
        # Encrypt the session key with the publick RSA key
        ...
        return enc_session_key + cipher.nonce + tag + cipher_text
